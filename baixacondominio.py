...
SECRETS_DIR = f'{os.path.abspath(os.path.dirname(__file__))}/secrets'
TOKEN_JSON = f'{SECRETS_DIR}/token.json'
CREDENTIALS_JSON = f'{SECRETS_DIR}/credentials.json'
DOWNLOADS_DIR = f'{os.path.abspath(os.path.dirname(__file__))}/downloads'
                ...
                for part in message['payload']['parts']:
                    if part['filename']:
                        ...
                        if match:
                            ...
                            file_name = f'BOLETO-{file_year}-{file_month}-{file_day}'
                        else:
                            match = re.match(r'ESI\_([0-9]{4})\_([0-9]{2})\-([0-9]{4})\.pdf', file_name)
                            if match:
                                file_name = f'EXTRATO-{match.group(3)}-{match.group(2)}'
